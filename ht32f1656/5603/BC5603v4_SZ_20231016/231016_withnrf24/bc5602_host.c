...
#if   _FRONTEND_V_ == 23 // 5602 v3 fullmask
static void init_setting_5602v3_DTR(void)
{
}
#endif
 ...
 *  @ingroup SPI_Commands_label
 */
static void spi_write_2fpga_rxg(void)
{
//  macroCSN_SELECT_FPGA;
//  spi_cmd0_20_set_register_bank(2);
//  spi_w(0x21, RXG_GAIN);            //RXG    RX Path Gain setting PGA,BPF,MIX,LNA
}



static void init_1adc(void)
{
...
//bank 2
    spi_cmd0_20_set_register_bank(2);
		...
		spi_w(0x29, 0x90);              //OD1     [5]  RESET_DSM: Reset DSM modulator 

		spi_w(0x2A, 0x11);			        //OD2     [7:6]DLY_SXPD   00:55us, 01:70us, 10:90us, 11:127us
    spi_w(0x2B, 0x12);              //VC1
		//spi_w(0x2C, 0x26);              //VC2
    ...
    spi_w(0x30, 0x08);              //RX3
		spi_w(0x31, 0x20);              //RX4
		spi_w(0x32, 0x08);              //RX5
		spi_w(0x33, 0x14);			       //TX1     [0]  PAD_EN: TX PA driver enable
	...
	//	spi_w(0x37, 0x50);              //CA1			
		...
		spi_w(0x3A, 0x54);              //LD1: SXLDO_EN[7:6]=01(1.5V),VCOLDO_EN[5:4]=01(1.5V),XOLDO[3:2]=01(1.5V)
		spi_w(0x3B, 0x54);							//LD2: DIG_LDO[7:6]=01(1.2V), IFLDO_EN[5:4]=01(1.5V),ADCLDO_EN[3:2]=01(1.5V)	  57
		spi_w(0x3C, 0x43);							//LD3: IB_BG[1:0]=2 LNALDO_EN[2:1]=01(1.5V) with Reg34=6F
		spi_w(0x3D, 0x10);							//XO1
		spi_w(0x3E, 0x02); 					  //XO2
                                    //        [7:5]-
                                    //        [4]  XO_SEL
...
static void init_2fpga(void)
{
    ...
    //
  //spi_w(0x02, 0x00);              //RC2     [0]  TX_FULL (RO read only)
    spi_w(0x02, 0x30);              //RC2     [0]  TX_FULL (RO read only)
                                    //        [3:1]RX_P_NO (RO read only)
                                    //      v3[4]  PSAVE  Power saving mode control. XCLK's gating clock
                                    //      v3[5]  PSAVE1 Power saving mode control. XCLK's gating clock for RFCTRL
                                    //        [7:6]-
    spi_w(0x03, 0x7F);              //MASK (00_CONFIG) [7]-, [6]MASK_RX, [5]MASK_TX, [4]MASK_MAX_RT, [3]MASK_ADDRESS, [2]MASK_BCMATCH, [1]MASK_RX_TO, [0]PRIM_RX
                                    //        [7]  -
                                    //        [6]  MASK_RX
                                    //        [5]  MASK_TX
                                    //        [4]  MASK_MAX_RT
    ...
    spi_w(0x04, 0x7E);              //IRQ1 (07_STATUS) [7]-, [6]RX_DR,   [5]TX_DS,   [4]MAX_RT, [3]ADDRESS, [2]BCMATCH, [1]RX_TO, [0]-
                                    //        [7]  -
                                    //        [6]  RX_DR
                                    //        [5]  TX_DS
                                    //        [4]  MAX_RT
  ...
  //spi_w(0x05, 0x__);              //IRQ2 (17_FIFO_STATUS) (RO read only)
                                    ...
                                    //        [1]  RX_FULL
                                    //        [0]  RX_EMPTY
  //spi_w(0x06, 0x00);              //PKT4 RO (strobe R_RX_PL_WID)
                                    //        [7:0](RO)RXDLEN
    spi_w(0x07, 0x00);              //RF_CH (05_RF_CH)
                                    //        [7]  -
                                    //        [6:0]RF_CH
    spi_w(0x08, 0xF0);              //RT1 (04_SETUP_RETR)
                                    //        [7:4]ARD: auto retransmit delay
                                    //        [3:0]ARC: auto retransmit count
  //spi_w(0x09, 0x__);              //RT2 (08_OBSERVE_TX)(Read-Only)
                                    //        [7:4](RO)CNT_PLOS
                                    //        [3:0](RO)CNT_ARC
  ...
  //spi_w(0x0D, 0x__);              //WL0
  if(dpl2_en_dpl==0)
    spi_w(0x0E, 0x00);              //DPL1 (1C_DYNPD)
  else
    spi_w(0x0E, DPL_P5 | DPL_P4 | DPL_P3 | DPL_P2 | DPL_P1 | DPL_P0);    //DPL1 (1C_DYNPD)
                                    //        [7:6]-
                                    //        [5]  DPL_P5
                                    //        [4]  DPL_P4
                                    //        [3]  DPL_P3
                                    ...
                                    //        [1]  DPL_P1
                                    //        [0]  DPL_P0
    spi_w(0x0F, en_inv_noack|dpl2_en_dpl|dpl2_en_ack_pld|dpl2_en_dyn_ack);  //DPL2 (1D_FEATURE)
                                    //        [7]  INV_NOACK
                                    //        [6:3]-
                                    //        [2]  EN_DPL    : enable dynamic payload length
                                    //        [1]  EN_ACK_PLD: enable ack_packet with payload
                                    //        [0]  EN_DYN_ACK: enable W_TX_PAYLOAD_NOACK command
    spi_w(0x10, 0x01);              //RXPW0 (11_RX_PW_P0)
                                    //        [7:6]-
                                    //        [5:0]RX_PW_P0
    spi_w(0x11, 0x02);              //RXPW1 (12_RX_PW_P1)
                                    //        [7:6]-
                                    //        [5:0]RX_PW_P1
    spi_w(0x12, 0x0B);              //RXPW2 (13_RX_PW_P2)
                                    //        [7:6]-
                                    //        [5:0]RX_PW_P2
    spi_w(0x13, 0x16);              //RXPW3 (14_RX_PW_P3)
                                    //        [7:6]-
                                    //        [5:0]RX_PW_P3
    spi_w(0x14, 0x1F);              //RXPW4 (15_RX_PW_P4)
                                    //        [7:6]-
                                    //        [5:0]RX_PW_P4
    spi_w(0x15, 0x20);              //RXPW5 (16_RX_PW_P5)
                                    //        [7:6]-
                                    //        [5:0]RX_PW_P5
    spi_w(0x16, ENAA_P5 | ENAA_P4 | ENAA_P3 | ENAA_P2 | ENAA_P1 | ENAA_P0);
                                    //ENAA (01_EN_AA)
                                    //        [7:6]-
                                    //        [5]  ENAA_P5
                                    //        [4]  ENAA_P4
    ...
    spi_w(0x17, 0x72);              //P2B0 (0C_RX_ADDR_P2)
                                    //        [7:0]P2B0
    spi_w(0x18, 0x73);              //P3B0 (0D_RX_ADDR_P3)
                                    //        [7:0]P3B0
    spi_w(0x19, 0x74);              //P4B0 (0E_RX_ADDR_P4)
                                    //        [7:0]P4B0
    spi_w(0x1A, 0x75);              //P5B0 (0F_RX_ADDR_P5)
                                    //        [7:0]P5B0

//bank 0
    spi_cmd0_20_set_register_bank(0);
  //spi_w(0x20, 0x__);              //OM
    spi_w(0x21, 0x0F);              //DM0     [6]  AMBLE2
    spi_w(0x22, 0xA0|dm1_aw|dm1_d_rate); //DM1
                                    //DM1     [7:4]RSSI_CTHD  RSSI threshold for carrier detection
                                    //        [3:2]AW   11:5, 10:4, 01:3, 00:-      (03_SETUP_AW[1:0])
                                    //        [1:0]D_RATE     00:250 kbps, 01:1 Mbps

    spi_w(0x23, 0x08);              //DM2     [7:0]FD_HOLD
		spi_w(0x24, 0x80);//default 80  //DM3     [7:0]PHASE_DSTEP


   #if   _HOST_TESTMODE_ == TRMODE_PTX
    spi_w(0x25, 0x01);              //DM4  (02_EN_RXADDR)
   #elif _HOST_TESTMODE_ == TRMODE_PRX
    spi_w(0x25, P5ACTIVE | P4ACTIVE | P3ACTIVE | P2ACTIVE | P1ACTIVE | P0ACTIVE); //DM4  (02_EN_RXADDR)
   #endif
                                    //        [7:6]-
                                    //        [5]  P5ACTIVE
                                    //        [4]  P4ACTIVE
                                    ...
                                    //        [1]  P1ACTIVE
                                    //        [0]  P0ACTIVE
  //spi_w(0x26, 0x__);              //STA1
                                    //        [7]  CD_FLAG Carrier Detection flag
                                    //        [6:3]-
                                    //        [2:0]OMST
  //spi_w(0x27, 0x__);              //RSSI2 (RO read only)
  //spi_w(0x28, 0x__);              //RSSI3 (RO read only)
    spi_w(0x29, pkt1_crc_sync | pkt1_crclength); //PKT1 (00_CONFIG.3.2)
    ...
    spi_w(0x2A, pkt2_wht_en|PKT2_WHT_SEED);
                                    //PKT2    [7]  WHT_EN: Data whitening enable, [6:0]WHTSD[6:0]:Data whitening seed
    spi_w(0x2B, PKT3_WHT_PCF_EN|(0<<3)|0x03);//PKT3    [7]  WHT_PCF_EN
    ...
    spi_w(0x2C, 0x01);              //PKT5    [7]-
                                    //        [6:4]DLY_RXS             0:4us, 1: 8us, 2:12us, 3:16us, 4:20us, 5:32us, 6: 64us, 7:100us
                                    //        [3]- 
                                    //        [2:0]DLY_TXS             0:40us,1:50us, 2:60us, 3:70us, 4:80us, 5:90us, 6:100us, 7:120us
  #if   PKT1_CRCLEN == 0x01
    spi_w(0x2D, 0x07);              //CRC1        CRC8  = X^8 + X^2 + X + 1
  #elif PKT1_CRCLEN == 0x02
    spi_w(0x2D, 0x21);              //CRC1        CRC16 = X^16 + X^12 + X^5 + 1
    spi_w(0x2E, 0x10);              //CRC2
  #elif PKT1_CRCLEN == 0x03
    spi_w(0x2D, 0x5B);              //CRC1        CRC24  x24 + x10 + x9 + x6 + x4 + x3 + x + 1
    spi_w(0x2E, 0x06);              //CRC2
    spi_w(0x2F, 0x00);              //CRC3
  #endif
    spi_w(0x30, 0xFF);              //CRC4 CRC_INI[23:16] CRC initial value 
    spi_w(0x31, 0xFF);              //CRC5 CRC_INI[15: 8] CRC initial value 
    spi_w(0x32, 0xFF);              //CRC6 CRC_INI[ 7: 0] CRC initial value 

//bank 1
    spi_cmd0_20_set_register_bank(1);
    spi_w(0x20, 0x04);              //AGC1    [1:0]AGC_CMP_THD[1:0]AGC comparison number threshold, 0:continuos AGC until SYNCWORD is detected, 1~3:comparison number thrshold
                                    //        [4:2]OFFSET_SEL[2:0]
                                    //        [7:5]-
    spi_w(0x21, 0x23);              //AGC2
                                    //        [2:0]IFDET_OK_THD[2:0]: IF detection OK threshold
                                    //      v3[7:3]OFFSET
  //spi_w(0x22, 0x__);              //AGC3(RO read only)
                                    //        [2:0]AGC_ST[2:0]:AGC state machine's state, 0x7:AGC_complete
                                    //        [3]-
                                    //        [7:4]GAIN_SEL[2:0]:gain curve select
    spi_w(0x23, 0x10);              //AGC4
...
//bank 2
  ...
  //spi_w(0x25, 0x20);//debug       //SX4     [5]  TX_MORE_EN=1
//    spi_w(0x26, 0xC1);              //CP1
//    spi_w(0x27, 0xFF);              //CP2   v3[3:0]CX_M
                                    //        [7:4]IB_CP_M
    spi_w(0x28, 0x01);              //CP3     [0]  ALPF_EN
                                    //        [2:1]PZ
                                    //        [4:3]P3RD
                                    //        [7:5]DLY_SYN[2:0] 000:16us, 001:20us, 010:24us, 011:28us, 100:32us, 101:36us, 110:40us, 111:100us
    spi_w(0x29, 0x03);              //OD1     [5]  RESET_DSM: Reset DSM modulator

    spi_w(0x2A, 0x01);//debug       //OD2     [7:6]DLY_SXPD   00:55us, 01:70us, 10:90us, 11:127us
    spi_w(0x2B, 0x03);              //VC1
  ...
  //spi_w(0x32, 0x08);              //RX5
    ...
    spi_w(0x33, 0x50);//debug       //TX1     [0]  PAD_EN: TX PA driver enable
                                    //        [3:1]RAMP_DN   000:x  001:30u  010:40u  011:50u  100:65u  101:80u  110:100u  111:120u
                                    //        [6:4]RAMP_UP   000:x  001:25u  010:35u  011:45u  100:60u  101:75u  110: 95u  111:115u
                                    //        [7]-
    spi_w(0x34, 0x63);// 0dbm       //TX2 duplicated bank0[0x3F]
                                    //        [7:5]CT_VDD_PAD
                                    //        [4:3]CT_PRE_PAD
                                    //        [2:0]CT_DRV_PAD
    spi_w(0x35, 0x21);// 0dbm       //TX3     [7]-
                                    //        [6:4]IBP_PAD[2:0]
                                    //        [3:0]IBN_PAD[3:0]
  ...
  //spi_w(0x37, 0x90);//calibrate   //CA1
    ...
    spi_w(0x38, 0x0A);              //CA2
    spi_w(0x39, 0x12);              //CA3  new[7]  FDCOC_EN
                                    //        [6:5]-
                                    //        [4:2]DLY_DCOCTB: DCOC turbo mode timing (switch RC) 000:0us, 001:2us, 010:4us, 011:5us, 100:6us, 101:7us, 110:8us, 111:10us
                                    //        [1]  DCOCTB_EN: DCOC turbo mode trigger as PWR_SOFT=1
                                    //        [0]  DCOC_ENB: RX DCOC (server loop) disable
  //spi_w(0x3D, 0x30);              //XO1 duplicated bank0[0x3d]
                                    //        [7:6]XSHIFT
                                    //        [5]  XO_IL
                                    //        [4:0]XO_TRIM

    spi_w(0x3E, 0x00); // may06     //XO2
...
//bank 3
    spi_cmd0_20_set_register_bank(3);
    spi_w(0x20, 0x80|0x05);         //IO1     [5:3]GIO2S[2:0]=0h:-     , [2:0]GIO1S[2:0]=0h:-      , [7:6]PADDS[1:0] PAD drive strength:0:0.5mA,1:1mA,2:5mA,3:10mA
  #if   _SPI_WIRES_ == 3
    spi_w(0x21, 0x00);              //IO2     [7:4]GIO4S[3:0]=5h:IRQ3  , [3:0]GIO3S[3:0]=0h:-
  #elif _SPI_WIRES_ == 4
    spi_w(0x21, 0x10);//4-wire SPI  //IO2     [7:4]GIO4S[3:0]=5h:IRQ3  , [3:0]GIO3S[3:0]=0h:-
  #endif
	
		
		spi_w(0x31, 0x04);              //LB_OFFSET_SEL	
		spi_w(0x36, 0x47);              //YAVG_FSEL=0; GAIN_MSWT;1'B0;ABORT=7us   
		spi_w(0x33, 0x50);              //PDIFF_THD
		spi_w(0x38, 0x14);              //GCV1_RSTHD	
		
    spi_w(0x26, 0x80|0x03);         //TEST4  de-modulator   GIO[5:0] = {ID_MATCH_OK(out),RBCLK(out),BDATA_OUT(out),CGATE_EN(out),DEMOD_EN(out), SAMPLE_EN(out)}

    spi_w(0x2B, 0x02);              //MOD1    DTR[7:0]    DTR[8:0]=0x001, Data Rate = Fxtal /32/(DTR+1) = 16000000 / (32*2) = 250K
    spi_w(0x2C, 0x00);              //MOD2    [7:5]-
                                    //        [4]  DITHER_EN
                                    //        [3:2]DITHER
                                    //        [1]  DTR_EN
                                    //        [0]  DTR[8]
  __read_fpga_all_ctrl_registers();
//  __check_v1_all_ctrl_registers();
...
//bank 0
    spi_cmd0_20_set_register_bank(0);
    if((zzz++)&0x01)
    spi_w(0x61, 0x4F);              //CFO1    [6]  AMBLE2
    else
    spi_w(0x61, 0x0F);              //CFO1    [6]  AMBLE2
}*/
...
void host5602_process(void)
{
        ...
        pkt1_crc_en   = PKT1_CRCEN ; //setting @bc5602_host.h
        pkt1_crc8_en  = PKT1_CRC8EN;      //setting @bc5602_host.h
        pkt2_wht_en     = PKT2_WHT_EN;      //setting @bc5602_host.h
        dpl2_en_dyn_ack = DPL2_EN_DYN_ACK;  //setting @bc5602_host.h
        dpl2_en_ack_pld = DPL2_EN_ACK_PLD;  //setting @bc5602_host.h
        dpl2_en_dpl     = DPL2_EN_DPL;      //setting @bc5602_host.h
        en_inv_noack    = DPL2_EN_INV_NOACK;//setting @bc5602_host.h
