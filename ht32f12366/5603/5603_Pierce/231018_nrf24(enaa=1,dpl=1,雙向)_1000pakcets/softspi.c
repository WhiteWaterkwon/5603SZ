...
#define SPI_CSN_1_SET_1     GPIO_WriteOutBits(HT_GPIOD, GPIO_PIN_1, SET)
...
#define SPI_CSN_1_RESET_0   GPIO_WriteOutBits(HT_GPIOD, GPIO_PIN_1, RESET)
...
//efine SPI_CSN_1_SET_1     HT_GPIOD->SRR = GPIO_PIN_1
...
//efine SPI_CSN_1_RESET_0   HT_GPIOD->RR = GPIO_PIN_1
...
static unsigned char spi_octet(unsigned char octet)
{
        ...
        return HT_SPI0->DR ;
				...
				else if(CSNSelect&0x02){
				HT_SPI1->DR = octet;  //SPI_SendData(HT_SPI0, octet);
        while( ((HT_SPI1->SR & SPI_FLAG_TXE)==(u32)RESET)?(SET):(RESET) );  //while (!SPI_GetFlagStatus(HT_SPI0, SPI_FLAG_TXE));
        return HT_SPI1->DR ;
				}
				else{
				HT_SPI0->DR = octet;  //SPI_SendData(HT_SPI0, octet);
