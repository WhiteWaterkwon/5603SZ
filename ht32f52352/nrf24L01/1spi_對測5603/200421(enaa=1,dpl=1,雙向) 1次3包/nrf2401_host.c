...
{
    unsigned char i;
    unsigned char status7, rrr,send_3_times;
    ReceiveResults res;
...
rf_SendData(0, str2, cnt);
						while(!(rf_GetStatus() & _BV(TX_FULL))){
...
//uart_putchar_n('\n');//debug
          //ce_15us_high_then_low();
	//		uart_puts(" RF_SETUP=");
		//	uart_putu8(rf_ReadRegister(RF_SETUP));
	for(send_3_times=0;send_3_times<3;send_3_times++){
...
//ce_15us_high_then_low();
				...
				// delay_unit_50us(30); //200*50us=10ms > 6.3ms (remote 5602 uart print take ~6.3ms)
				
			}
					spi_SetCEState(FALSE);
				while(!(rf_GetFifoStatus() & _BV(TX_EMPTY))){
				rf_FlushBuffer(e_flush_xmit);		
				}			
				uart_puts("/** ");
						while(!(rf_GetStatus() & _BV(TX_FULL))){
