...
/* Global functions ----------------------------------------------------------------------------------------*/
static unsigned int count_send ;
static unsigned int count_success ;
static unsigned int count_fail ;
...
static void foo_ptx_print_count_send(unsigned char istxds)
{
    ...
    uart_puts("count_send=");   uart_putu16(count_send);
...
//		uart_puts("111");//2
    #if _HOST_TESTMODE_ == 2	//PTX		
		sprintf(text, "%02d%01d%04d%04d",length,cmd, count_send,count_success);//4  tx
		#elif _HOST_TESTMODE_ == 3   //PRX
		sprintf(text, "%02d%01d%04d%04d",length,cmd, count_success,count_fail);//4   rx
		#endif
		uart_puts(text);  
		uart_putchar_n('\n');			
		}
		else if(cmd==2) { //mod 2 restart
		char text[15],length;
		length = 2+1+1;
		sprintf(text, "%02d%01d%d",length,cmd,0);
		count_send=0;
		count_txds=0;
		count_maxrt=0;
		count_success=0;	
    #if _HOST_TESTMODE_ == 2				
		hfsm1.trmode = TRMODE_TX32;	//  tx
		#elif _HOST_TESTMODE_ == 3   //PRX
		hfsm1.trmode = TRMODE_RX32;	
		#endif
		uart_puts(text); 
...
uart_puts(" MAXRT  SETUP_RETR=");  rrr=rf_ReadRegister(SETUP_RETR);  uart_putu8(rrr); uart_puts("  OBSERVE_TX=");  rrr=rf_ReadRegister(OBSERVE_TX);  uart_putu8(rrr); //uart_putchar_n('\n');//debug
                    #if TOTAL_NUM_TEST_PKTS != 0
                    if ( count_send >= TOTAL_NUM_TEST_PKTS ) {
                        spi_SetCEState(FALSE);
                        rf_ResetStatus(e_mode_both);
                        rf_FlushBuffer(e_flush_both);
                        hfsm1.trmode=0;
...
//ce_15us_high_then_low();
        ...
        delay_unit_50us(500); //200*50us=10ms > 6.3ms (remote 5602 uart print take ~6.3ms)
...
uart_puts("nrf24L01+  PRX   CH="); uart_putu8(DEF_RF_CHAN); uart_putchar_n('\n');
  ...
  //rf_Init(DEF_RF_CHAN, DEF_PAYLOAD_SIZE, hfsm1.rfdatarate, FALSE); //FALSE: _use_dynamic_payload =0
    rf_ConfigurePipe(0, pipe0_addr, RXPWP0); // RXPWP0 unused in DPL=1 mode
    rf_ConfigurePipe(1, pipe1_addr, RXPWP1); // RXPWP1 unused in DPL=1 mode
    rf_ConfigurePipe(2, pipe2_addr, RXPWP2); // RXPWP2 unused in DPL=1 mode
    rf_ConfigurePipe(3, pipe3_addr, RXPWP3); // RXPWP3 unused in DPL=1 mode
    rf_ConfigurePipe(4, pipe4_addr, RXPWP4); // RXPWP4 unused in DPL=1 mode
    rf_ConfigurePipe(5, pipe5_addr, RXPWP5); // RXPWP5 unused in DPL=1 mode
...
uart_puts("count_success=");   uart_putu16(++count_success);
