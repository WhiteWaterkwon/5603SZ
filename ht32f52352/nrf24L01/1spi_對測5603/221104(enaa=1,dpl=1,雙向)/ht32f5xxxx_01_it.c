...
void USART1_IRQHandler(void)
{
  ...
  /* Tx, move data from buffer to USART FIFO                                                                */
  if (USART_GetFlagStatus(HT_USART1, USART_FLAG_TXDE))    // offset 0x010: USART Status and Interrupt Flag register
                                   //[7]transmit_data_FIFO Empty
  {
    if (IS_BUFFER_EMPTY(UR1TxReadIndex, UR1TxWriteIndex))
    {
      USART_IntConfig(HT_USART1, USART_INT_TXDE, DISABLE);    // @arg USART_INT_TXDE   / USART_IER_THREIE :
    }
    else
    {
      for (i = 0; i < 8; i++)  //            ht32f12366 8 bytes transmit_data_FIFO
                               ...
                               // then can write max 8 bytes to transmit_data_FIFO
      {
        USART_SendData(HT_USART1, UR1TxBuf[UR1TxReadIndex]);
  ...
  if (USART_GetFlagStatus(HT_USART1, USART_FLAG_RXDR))
  {
    ...
    UR1RxBuf[UR1RxWriteIndex] = USART_ReceiveData(HT_USART1);
